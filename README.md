...
*  **org.ros.rosjava\_messages.\***: there are three sets of the java messages generated by the `ros:generate` command, they are used by the `be.iminds.iot.simulator.gazebo` project when generating the Gazebo-specific commands, in additionally you can download the needed message jars from the `rosjava_messages` project, but in case of the difference of Ros and Gazebo version result in the defference with the ros messages definition, a md5 error will accour, so you have to replace the message type class with the one generated by the `ros_generate` command.

    In the `org.ros.rosjava_messages.gazebo_msgs` project, the `GetModelState.java` definition have to be replaced by the `GetModelState.java` in generated_msgs folder by the `ros:generate` command
