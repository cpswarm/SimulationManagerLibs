...
import it.ismb.pert.cpswarm.simulationOrchestrator.GetProgressSender;
...
public final class MessageEventCoordinatorImpl implements IncomingChatMessageListener {
	...
	public MessageEventCoordinatorImpl(final SimulationOrchestrator orchestrator) {
		...
		System.out.println("create a   MessageEventCoordinatorImpl ");
	}
	...
	public void newIncomingMessage(EntityBareJid sender, Message msg, org.jivesoftware.smack.chat2.Chat chat) {
		...
		System.out.println("\n SOO RECEIEVS MSG =  " + msg.getBody()+" \n");
		eu.cpswarm.optimization.messages.ReplyMessage message = serializer.fromJson(msg.getBody());
		if (sender.toString().startsWith("manager")) {
			if (message instanceof SimulatorConfiguredMessage) {
				...
				if (message instanceof SimulationResultMessage) {
					System.out.println("SOO MessageEventCoordinatorImpl received a SimulationResultMessage = "
		...
		//		System.out.println("SOO MessageEventCoordinatorImpl received a OptimizationProgressMessage =  "+ (OptimizationProgressMessage) message);
				...
				if (message instanceof OptimizationStartedMessage) {
					...
					if (message instanceof OptimizationCancelledMessage) {
						System.out.println("Reply received: OptimizationCancelledMessage.....");
						OptimizationCancelledMessage reply = (OptimizationCancelledMessage) serializer
								.fromJson(msg.getBody());
						// TODO
					}
				}
			}
	...
	private void handleOptimizationStartedMessage(OptimizationStartedMessage reply) {
		System.out.println("SOO MESSAGECOOR LINE 79 handleOptimizationStartedMessage......... ...." + "   monitor = "
				+ parent.getMonitoring().booleanValue());
		if (reply.getOperationStatus().equals(Status.OK) && reply.getId().equals(parent.getOptimizationId())
				&& parent.getMonitoring().booleanValue()) {
			System.out.println("LINE 84  GetProgressSender.....");
	...
	private void handleOptimizationProgressMessage(OptimizationProgressMessage progress, MessageSerializer serializer) {
		System.out.println("Optimization " + progress.getId() + ", progress:" + progress.getProgress()
				+ "%, fitness value: " + progress.getFitnessValue());
