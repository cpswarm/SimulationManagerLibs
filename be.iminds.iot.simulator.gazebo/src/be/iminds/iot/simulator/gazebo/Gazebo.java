...
public class Gazebo implements Simulator {
	...
	public synchronized void stop() {
		...
		try {
			deferred.getPromise().getValue();  // = null if resolved successfully
		} catch (Exception e) {       /*this Promise was resolved with a failure, or If the current thread was interrupted while the getValue() method must block and wait for this Promise to be resolved before completing*/
			e.printStackTrace();
		}
	}
	...
	public void loadScene(String file, Map<String, String> config) {
		...
		if(config!=null && config.containsKey("pose")){
